...
- **OperatorIntent vs RuntimeIdentity vs LatestAvailable — three concepts, never collapse.** The Quadlet's `Image=` line is _OperatorIntent_ ("what should podman start next?") and can be a floating ref like `:latest`. dockerode's `inspect().Image` returns a sha256 digest with no path back to the original tag, so it is NEVER a reliable answer to "what version is running?" The engine's own `/api/health.version` (loaded from package.json at boot) is _RuntimeIdentity_ — honest, always a clean semver. GHCR's tag list is _LatestAvailable_. `src/runtime-version.ts` is the single resolver: health-probe → OCI image label (`org.opencontainers.image.version`, set at publish time) → Quadlet tag (only counts as version when semver-shaped). state.ts, update-checker.ts, routes/doctor.ts, routes/self.ts all go through this resolver — DO NOT extract tags from `inspect().Image` ever again. Incident 2026-05-25 13:00 CEST: a 4th call site in state.ts kept showing digests in the Updater card after three other call sites had been fixed; the refactor introduced the resolver as the single source of truth.
- **Cache-invalidate on engine update.** Successful `POST /api/self/update` (just before `restartUnit`) and successful `performDoctorSwitch` (after `writeLastGood`) both call `update-checker.invalidate()` to bust the in-memory `/api/updates/available` cache. Closes the "I just released 0.6.4, why does the badge still say 0.6.3" publish-day window without waiting for the 24h tick.
- **Daily update check.** `src/update-checker.ts` runs a `setInterval` every 24h (boot-time check is best-effort) that re-queries GHCR for both peer engines' latest stable tags and stores the result in a module-level cache. `GET /api/updates/available` exposes the cache (no auth — read-only view over an already-cached GHCR result), and the App-level webapp badge polls it every 5 min so a user not on the Dashboard tab still sees the notification. **No auto-apply** — boats have unreliable connectivity and surprising restarts during a voyage are dangerous. The comparison is against `health.version` (RuntimeIdentity), not the Quadlet tag, so a `:latest`-pinned install still correctly surfaces semver upgrades.
...
| `src/update-checker.ts`               | Daily setInterval that re-queries GHCR for both peer engines. Cache feeds `/api/updates/available`. Exposes `invalidate()` for post-engine-update cache busts. Compares `health.version` (NOT Quadlet tag) against GHCR.   |
| `src/runtime-version.ts`              | RuntimeIdentity resolver: health-probe → OCI image label → Quadlet tag fallback. Single source of truth for "what version is this container running." All version-display code goes through `getRuntimeIdentity()`.        |
